...
def generate_ellipsoid_clusters(distance, n_samples=100, cluster_std=0.5):
    ...
    # Generate the second cluster (class 1) with the shift
    # Direction vector along y = -x (normalized)
    direction = np.array([1, -1])
    direction = direction / np.linalg.norm(direction)  # Normalize the direction vector
    
    # Apply the shift to the second cluster (class 1)
    X2_shifted = X2 + distance * direction  

    # Combine the clusters into one dataset
    X = np.vstack((X1, X2_shifted))
    y = np.hstack((y1, y2))
    return X, y
...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        #Record all necessary information for each distance & store coefficients
    ...
    #Plot beta0
    plt.subplot(3, 3, 1)  # Creating a 3x3 grid, plot in the first position
    ...
    #plt.ylim(-2, 0)  # Setting y-axis limits to the range of interest
    plt.grid(True)
    ...
    # Plot logistic loss
    plt.subplot(3, 3, 6)
    plt.plot(shift_distances, loss_list, marker='o', linestyle='-', color='purple')
